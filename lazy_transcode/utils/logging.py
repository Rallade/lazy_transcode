...
Provides consistent logging patterns with configurable debug levels and timestamps:
- [HH:MM:SS] [INFO] for general information
- [HH:MM:SS] [WARN] for warnings
- [HH:MM:SS] [ERROR] for errors  
- [HH:MM:SS] [RESULT] for final results
- [HH:MM:SS] [DEBUG] for debug information
- [HH:MM:SS] [VBR] for VBR optimization messages
- [HH:MM:SS] [VMAF] for VMAF calculation messages
- [HH:MM:SS] [CLEANUP] for cleanup operations
- And many domain-specific loggers

Usage:
    from lazy_transcode.utils.logging import get_logger, set_debug_mode, set_timestamps_enabled
    
    # Configure logging globally
    set_debug_mode(True)  # Enable debug messages
    set_timestamps_enabled(False)  # Disable timestamps
...
_QUIET_MODE = False
_LOG_LEVEL = "INFO"
_TIMESTAMPS_ENABLED = True  # Enable timestamps by default
...
def set_timestamps_enabled(enabled: bool):
    """Enable or disable timestamps in log messages"""
    global _TIMESTAMPS_ENABLED
    _TIMESTAMPS_ENABLED = enabled
...
class Logger:
    ...
    def _log(self, level: str, message: str, prefix: str = ""):
        ...
        print(f"{timestamp_str}[{level}] {full_prefix}{message}")
    ...
    def result(self, message: str):
        ...
        if self._should_log(LogLevel.INFO):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[RESULT] {self.prefix}{message}")
    ...
    def vbr(self, message: str):
        ...
        if self._should_log(LogLevel.INFO):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[VBR] {message}")
    ...
    def vbr_debug(self, message: str):
        ...
        if _DEBUG_ENABLED and self._should_log(LogLevel.DEBUG):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[VBR-DEBUG] {message}")
    ...
    def vbr_bisect(self, message: str):
        ...
        if self._should_log(LogLevel.INFO):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[VBR-BISECT] {message}")
    ...
    def vbr_bounds(self, message: str):
        ...
        if self._should_log(LogLevel.INFO):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[VBR-BOUNDS] {message}")
    ...
    def vbr_test(self, message: str):
        ...
        if self._should_log(LogLevel.INFO):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[VBR-TEST] {message}")
    ...
    def vbr_cache(self, message: str):
        ...
        if self._should_log(LogLevel.INFO):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[VBR-CACHE] {message}")
    ...
    def vbr_early_exit(self, message: str):
        ...
        if self._should_log(LogLevel.INFO):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[VBR-EARLY-EXIT] {message}")
    ...
    def vbr_abandon(self, message: str):
        ...
        if self._should_log(LogLevel.WARN):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[VBR-ABANDON] {message}")
    ...
    def vbr_skip(self, message: str):
        ...
        if self._should_log(LogLevel.INFO):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[VBR-SKIP] {message}")
    ...
    def vbr_error(self, message: str):
        ...
        if self._should_log(LogLevel.ERROR):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[VBR-ERROR] {message}")
    ...
    def vbr_success(self, message: str):
        ...
        if self._should_log(LogLevel.INFO):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[VBR-SUCCESS] {message}")
    ...
    def vbr_failed(self, message: str):
        ...
        if self._should_log(LogLevel.WARN):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[VBR-FAILED] {message}")
    ...
    def vmaf(self, message: str):
        ...
        if self._should_log(LogLevel.INFO):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[VMAF] {message}")
    ...
    def vmaf_debug(self, message: str):
        ...
        if _DEBUG_ENABLED and self._should_log(LogLevel.DEBUG):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[VMAF-DEBUG] {message}")
    ...
    def vmaf_cmd(self, message: str):
        ...
        if _DEBUG_ENABLED and self._should_log(LogLevel.DEBUG):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[VMAF-CMD] {message}")
    ...
    def vmaf_threads(self, message: str):
        ...
        if _DEBUG_ENABLED and self._should_log(LogLevel.DEBUG):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[VMAF-THREADS] {message}")
    ...
    def cleanup(self, message: str):
        ...
        if self._should_log(LogLevel.INFO):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[CLEANUP] {message}")
    ...
    def discovery(self, message: str):
        ...
        if self._should_log(LogLevel.INFO):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[DISCOVERY] {message}")
    ...
    def encoder(self, message: str):
        ...
        if self._should_log(LogLevel.INFO):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[ENCODER] {message}")
    ...
    def output(self, message: str):
        ...
        if self._should_log(LogLevel.INFO):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[OUTPUT] {message}")
    ...
    def mode(self, message: str):
        ...
        if self._should_log(LogLevel.INFO):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[MODE] {message}")
    ...
    def hdr(self, message: str):
        ...
        if self._should_log(LogLevel.INFO):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[HDR] {message}")
    ...
    def cmd(self, message: str):
        ...
        if _DEBUG_ENABLED and self._should_log(LogLevel.DEBUG):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[CMD] {message}")
    ...
    def sample_extract(self, message: str):
        ...
        if _DEBUG_ENABLED and self._should_log(LogLevel.DEBUG):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[SAMPLE-EXTRACT] {message}")
    ...
    def sample_encode(self, message: str):
        ...
        if _DEBUG_ENABLED and self._should_log(LogLevel.DEBUG):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[SAMPLE-ENCODE] {message}")
    ...
    def vbr_encode(self, message: str):
        ...
        if _DEBUG_ENABLED and self._should_log(LogLevel.DEBUG):
            timestamp = self._get_timestamp()
            timestamp_str = f"[{timestamp}] " if timestamp else ""
            print(f"{timestamp_str}[VBR-ENCODE] {message}")
...
def log_info(message: str):
    ...
    if not _QUIET_MODE:
        timestamp = datetime.now().strftime("%H:%M:%S") if _TIMESTAMPS_ENABLED else ""
        timestamp_str = f"[{timestamp}] " if timestamp else ""
        print(f"{timestamp_str}[INFO] {message}")

def log_warn(message: str):
    """Log a warning message"""
    timestamp = datetime.now().strftime("%H:%M:%S") if _TIMESTAMPS_ENABLED else ""
    timestamp_str = f"[{timestamp}] " if timestamp else ""
    print(f"{timestamp_str}[WARN] {message}")

def log_error(message: str):
    """Log an error message"""
    timestamp = datetime.now().strftime("%H:%M:%S") if _TIMESTAMPS_ENABLED else ""
    timestamp_str = f"[{timestamp}] " if timestamp else ""
    print(f"{timestamp_str}[ERROR] {message}")
...
def log_gradient_descent(message: str):
    ...
    if not _QUIET_MODE:
        timestamp = datetime.now().strftime("%H:%M:%S") if _TIMESTAMPS_ENABLED else ""
        timestamp_str = f"[{timestamp}] " if timestamp else ""
        print(f"{timestamp_str}[GRADIENT-DESCENT] {message}")
...
def log_step(step_number: int, message: str):
    ...
    if not _QUIET_MODE:
        timestamp = datetime.now().strftime("%H:%M:%S") if _TIMESTAMPS_ENABLED else ""
        timestamp_str = f"[{timestamp}] " if timestamp else ""
        print(f"{timestamp_str}[STEP {step_number}] {message}")
...
def log_setup(message: str):
    ...
    if not _QUIET_MODE:
        timestamp = datetime.now().strftime("%H:%M:%S") if _TIMESTAMPS_ENABLED else ""
        timestamp_str = f"[{timestamp}] " if timestamp else ""
        print(f"{timestamp_str}[SETUP] {message}")
...
def log_progress(message: str):
    ...
    if not _QUIET_MODE:
        timestamp = datetime.now().strftime("%H:%M:%S") if _TIMESTAMPS_ENABLED else ""
        timestamp_str = f"[{timestamp}] " if timestamp else ""
        print(f"{timestamp_str}[PROGRESS] {message}")
